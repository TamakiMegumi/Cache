...
namespace CacheSpace
{
    ...
    public:
        explicit ArcCache(size_t capacity = 10, size_t transformThershold = 2)
            : capacity(capacity), transformThershold(transformThershold),
              lruPart(std::make_unique<ArcLRUPart<Key, Value>>(capacity, transformThershold)),
              lfuPart(std::make_unique<ArcLFUPart<Key, Value>>(capacity, transformThershold))
        {
        }
        ...
        bool get(Key key, Value &val) override
        {
            ...
            if (lruPart->get(key, val, transformThreshold))
            {
                if (shouldTransform)
                {
                    lfuPart->put(key, val);
